...
testDbplyrFunctions <- function(connectionDetails, cdmDatabaseSchema) {
  ...
  nMales <- person %>%
    filter(as.double(gender_concept_id) == 8507) %>%
    count() %>%
    pull()
  expect_gt(nMales, 1)

  nMales2 <- person %>%
    filter(as.double(gender_concept_id) %in% c(8507)) %>%
    count() %>%
    pull()
  expect_gt(nMales2, 1)
  ...
  sexString <- person %>%
    mutate(sex = ifelse(gender_concept_id == 8507, "Male",
  ...
  if (!isDremio) {
    cars2 <- copy_to(connection, cars, overwrite = TRUE) %>% collect()
